    ...
    <div class="p-6 bg-white border-b border-gray-200">
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            <p class="text-sm text-gray-600">
                ...
                <span class="text-gray-800">{{ $organizationUnit->full_path }}</span>
            </p>
        </div>

        <form action="{{ route('organization-units.update', $organizationUnit) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $organizationUnit->code) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6" style="position: relative; z-index: 30;">
                    ...
                    <x-searchable-dropdown
                        ...
                        value-field="id"
                        label-field="name"
                        :selected="old('type_id', $organizationUnit->type_id)"
                    ...
                    <x-searchable-dropdown
                        ...
                        label-field="name"
                        group-field="group"
                        :selected="old('parent_id', $organizationUnit->parent_id)"
                ...
                <x-searchable-dropdown
                    ...
                    value-field="id"
                    label-field="name"
                    :selected="old('head_id', $organizationUnit->head_id)"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">{{ old('description', $organizationUnit->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $organizationUnit->is_active) ? 'checked' : '' }}
