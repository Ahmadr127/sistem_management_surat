    ...
    <div x-data="sidebarComponent()" x-init="init()" class="h-full flex overflow-x-hidden">
        ...
        <div class="sidebar fixed inset-y-0 left-0 z-50 bg-green-700 shadow-lg flex flex-col h-screen lg:static lg:inset-0"
            ...
            <nav class="flex-1 overflow-y-auto sidebar-scroll px-4 py-6">
                ...
                @if(auth()->user()->hasPermission('manage_users') || auth()->user()->hasPermission('manage_roles') || auth()->user()->hasPermission('manage_permissions') || auth()->user()->hasPermission('manage_jabatan'))
                <div class="mb-4" x-data="{ open: {{ request()->routeIs('manageuser.*') || request()->routeIs('roles.*') || request()->routeIs('permissions.*') || request()->routeIs('managejabatan.*') ? 'true' : 'false' }} }">
                <button @click="open = !open" 
                    ...
                    <div x-show="open" 
                        ...
                        <a href="{{ route('manageuser.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->routeIs('manageuser.*') ? 'bg-green-800 text-white' : '' }}"
                           title="Manage Users">
                        ...
                        @if(auth()->user()->hasPermission('manage_jabatan'))
                        <a href="{{ route('managejabatan.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->routeIs('managejabatan.*') ? 'bg-green-800 text-white' : '' }}"
                           title="Manage Jabatan">
                            <i class="fas fa-briefcase w-4 mr-2"></i>
                            <span class="sidebar-text">Manage Jabatan</span>
                        </a>
                        @endif
                    </div>
                </div>
                ...
                @if(auth()->user()->hasPermission('manage_organization_types') || auth()->user()->hasPermission('manage_organization_units') || auth()->user()->hasPermission('manage_perusahaan'))
                <div class="mb-4" x-data="{ open: {{ request()->routeIs('organization-types.*') || request()->routeIs('organization-units.*') || request()->routeIs('manageperusahaan.*') ? 'true' : 'false' }} }">
                <button @click="open = !open" 
                        class="w-full flex items-center justify-between px-4 py-3 text-white rounded-lg hover:bg-green-800 transition-colors">
                    <div class="flex items-center">
                    ...
                    <div x-show="open" 
                        ...
                        @if(auth()->user()->hasPermission('manage_perusahaan'))
                        <a href="{{ route('manageperusahaan.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->routeIs('manageperusahaan.*') ? 'bg-green-800 text-white' : '' }}"
                           title="Manage Perusahaan">
                            <i class="fas fa-building-columns w-4 mr-2"></i>
                            <span class="sidebar-text">Manage Perusahaan</span>
                        </a>
                        @endif
                ...
                <div class="mb-4" x-data="{ open: {{ request()->is('suratmasuk*') || request()->is('suratkeluar*') || request()->is('disposisi*') || request()->is('nomor*') || request()->is('surat-unit-manager*') ? 'true' : 'false' }} }">
                    <button @click="open = !open" 
                            class="w-full flex items-center justify-between px-4 py-3 text-white rounded-lg hover:bg-green-800 transition-colors">
                        <div class="flex items-center">
                            <i class="fas fa-envelope w-5 sidebar-icon mr-3"></i>
                            <span class="sidebar-text">Surat Menyurat</span>
                        </div>
                        <i class="fas fa-chevron-down sidebar-text text-xs transition-transform duration-200" :class="{ 'rotate-180': open }"></i>
                    </button>
                    ...
                    <div x-show="open" 
                        ...
                        @if(auth()->user()->hasPermission('manage_surat_masuk'))
                        <a href="{{ url('/suratmasuk') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->is('suratmasuk*') ? 'bg-green-800 text-white' : '' }}"
                           title="Surat Masuk">
                        ...
                        @if(auth()->user()->hasPermission('manage_surat_keluar'))
                        <a href="{{ route('suratkeluar.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->is('suratkeluar*') ? 'bg-green-800 text-white' : '' }}"
                           title="Surat Keluar">
                        ...
                        <a href="{{ route('nomor.generate') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->is('nomor*') ? 'bg-green-800 text-white' : '' }}"
                           title="Generate Nomor Surat">
                            <i class="fas fa-hashtag w-4 mr-2"></i>
                            <span class="sidebar-text">Generate Nomor</span>
                        </a>
                        @endif
                        
                        @if(auth()->user()->hasPermission('create_surat_unit'))
                        <a href="{{ route('surat-unit-manager.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->is('surat-unit-manager*') && !request()->is('surat-unit-manager/manager*') && !request()->is('surat-unit-manager/manager-keuangan*') ? 'bg-green-800 text-white' : '' }}"
                           title="Surat Unit Manager">
                            <i class="fas fa-file-alt w-4 mr-2"></i>
                            <span class="sidebar-text">Surat Unit Manager</span>
                        </a>
                        @endif
                        
                        @if(auth()->user()->hasPermission('approve_surat_unit'))
                        <a href="{{ route('surat-unit-manager.manager.index') }}" 
                           class="flex items-center px-3 py-2 text-green-100 rounded-lg hover:bg-green-800 hover:text-white transition-colors text-sm {{ request()->is('surat-unit-manager/manager*') || request()->is('surat-unit-manager/manager-keuangan*') ? 'bg-green-800 text-white' : '' }}"
                ...
                <div class="mb-4">
                    <a href="{{ route('laporan') }}" 
                       class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg hover:bg-green-800 transition-colors {{ request()->is('laporan*') ? 'bg-green-800' : '' }}" 
                       title="Laporan">
                        <i class="fas fa-chart-bar w-5 sidebar-icon mr-3"></i>
                ...
                <div class="mb-4">
                    <a href="{{ url('/arsip') }}" 
                       class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg hover:bg-green-800 transition-colors {{ request()->is('arsip*') ? 'bg-green-800' : '' }}" 
                       title="Arsip">
                        <i class="fas fa-archive w-5 sidebar-icon mr-3"></i>
