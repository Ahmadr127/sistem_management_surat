...
@section('title', 'Konversi ke Surat Keluar - SISM Azra')
...
@section('content')
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-white flex justify-between items-center">
            <div>
                <h2 class="text-lg font-semibold text-gray-800">Konversi ke Surat Keluar - {{ $suratUnit->nomor_surat }}</h2>
                <p class="text-xs text-gray-500 mt-1">Silakan sesuaikan data surat keluar jika diperlukan</p>
            </div>
        ...
        <!-- Form -->
        <form action="{{ route('surat-unit-manager.approval.convert', $suratUnit->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
                ...
                <div class="p-6 space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="text" name="nomor_surat" 
                                    ...
                                    value="{{ old('nomor_surat', $suratUnit->nomor_surat) }}">
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="date" name="tanggal_surat" required
                                    value="{{ old('tanggal_surat', $suratUnit->tanggal_surat->format('Y-m-d')) }}"
                                    class="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-green-500 focus:ring focus:ring-green-200 transition-all duration-200">
                            </div>
                            @error('tanggal_surat')
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select name="jenis_surat" id="jenis_surat" required
                                        ...
                                        <option value="internal" {{ old('jenis_surat', $suratUnit->jenis_surat) == 'internal' ? 'selected' : '' }}>
                                            Internal
                                        </option>
                                        <option value="eksternal" {{ old('jenis_surat', $suratUnit->jenis_surat) == 'eksternal' ? 'selected' : '' }}>
                                            Eksternal
                                        </option>
                                    </select>
                                    <div
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <select name="sifat_surat" required
                                    class="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-green-500 focus:ring focus:ring-green-200 transition-all duration-200 appearance-none bg-white">
                                    <option value="normal" {{ old('sifat_surat', $suratUnit->sifat_surat) == 'normal' ? 'selected' : '' }}>
                                        Normal
                                    </option>
                                    <option value="urgent" {{ old('sifat_surat', $suratUnit->sifat_surat) == 'urgent' ? 'selected' : '' }}>
                                        Urgent
                                    </option>
                                </select>
                                <div
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="perihal" required
                            class="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-green-500 focus:ring focus:ring-green-200 transition-all duration-200"
                            rows="3" placeholder="Masukkan perihal surat">{{ old('perihal', $suratUnit->perihal) }}</textarea>
                        @error('perihal')
                            <p class="text-red-500 text-xs">
                                <i class="ri-error-warning-line mr-1"></i>
                                {{ $message }}
            ...
            <div class="mt-6 bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <!-- Menampilkan Lampiran yang Ada -->
                    @if($suratUnit->files && $suratUnit->files->count() > 0)
                    <div class="mb-6">
                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Lampiran Saat Ini</h4>
                        <div class="space-y-2">
                            @foreach($suratUnit->files as $file)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                                <div class="flex items-center overflow-hidden">
                                    <i class="ri-file-text-line text-blue-500 text-xl mr-3"></i>
                                    <span class="text-sm text-gray-700 font-medium truncate">{{ $file->original_name }}</span>
                                </div>
                                <div class="flex space-x-2">
                                    <a href="{{ route('surat-unit-manager.preview-file', ['suratUnitManager' => $suratUnit->id, 'fileId' => $file->id]) }}" 
                                       target="_blank"
                                       class="px-2 py-1 bg-white border border-gray-300 text-gray-600 rounded text-xs hover:bg-gray-50 transition-colors">
                        ...
                        <p class="text-xs text-gray-500 mt-2"><i class="ri-information-line"></i> Lampiran ini akan otomatis disertakan ke dalam Surat Keluar.</p>
                    </div>
                    @endif

                    <div class="space-y-4">
                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Tambahkan Lampiran Baru (Opsional)</h4>
                        <!-- File Upload Area -->
                        <div class="w-full">
