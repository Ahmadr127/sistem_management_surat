...
<x-modal-dinamis id="edit-disposisi" title="Edit Disposisi Surat" size="lg" :showFooter="true">
    <x-slot:trigger>
    ...
    <form id="edit-disposisi-form" class="space-y-4">
        ...
        {{-- Bagian Sekretaris --}}
        <div id="sekretaris-section" class="border-b border-gray-200 pb-4">
        ...
        {{-- Bagian Direktur --}}
        <div id="direktur-section" class="border-b border-gray-200 pb-4">
        ...
        <div id="tujuan-disposisi-section">
    ...
    <x-slot:footer>
        <div class="flex justify-end space-x-3">
            ...
            <x-button variant="success" icon="ri-save-line" id="save-disposisi-button">
                Simpan Perubahan
            </x-button>
        </div>
    </x-slot:footer>
...
@push('scripts')
<script>
// Fungsi untuk membuka modal edit disposisi
window.openEditDisposisiModal = function(suratId) {
    // Trigger modal
    window.dispatchEvent(new CustomEvent('open-modal-edit-disposisi'));
    
    // Load data disposisi
    loadDisposisiData(suratId);
};

// Load disposisi data
async function loadDisposisiData(suratId) {
    try {
        // Show loading state
        ...
        if (data.success && data.disposisi) {
            ...
            if (userRole === 1) { // Sekretaris
                ...
                if (disposisi.id) {
                    loadTujuanDisposisi(disposisi.id);
                } else {
                    if (container) {
        ...
        // Close modal
        window.dispatchEvent(new CustomEvent('close-modal-edit-disposisi'));
...
async function loadTujuanDisposisi(disposisiId) {
    const container = document.getElementById('tujuan-disposisi-container');
...
document.addEventListener('DOMContentLoaded', function() {
    const saveBtn = document.getElementById('save-disposisi-button');
    if (saveBtn) {
        saveBtn.addEventListener('click', async function() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Reload data
                    if (window.suratKeluarIndex && window.suratKeluarIndex.loadSuratKeluar) {
                        setTimeout(() => window.suratKeluarIndex.loadSuratKeluar(), 1000);
                    }
                } else {
