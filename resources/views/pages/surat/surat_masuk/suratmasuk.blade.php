...
@section('content')
    ...
    <script>
        ...
        let userOrgUnitId = {{ auth()->check() && auth()->user()->organization_unit_id ? auth()->user()->organization_unit_id : 'null' }};
                    ...
                    .then(data => {
                        ...
                        if (userRole === 0 || userRole === 3) { // Staff atau Admin
                            ...
                            filteredData = data.filter(surat => {
                                // Cek apakah user adalah tujuan disposisi ATAU ada tujuan disposisi yang satu unit
                                const isTujuanDisposisi = surat.disposisi?.tujuan?.some(
                                    tujuan => tujuan.id === {{ auth()->id() }} || (userOrgUnitId !== null && tujuan.organization_unit_id === userOrgUnitId)
                                );
                            ...
                            <td class="px-2 py-2 whitespace-nowrap text-sm font-medium">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    ${(userRole === 1 || userRole === 2 || userRole === 5 || userRole === 8) ? `
                                        <button onclick="openEditDisposisiSuratMasuk(${surat.id})" class="inline-flex items-center px-2.5 py-1.5 bg-white border border-indigo-300 hover:bg-indigo-50 text-indigo-700 rounded-md shadow-sm transition-colors duration-200" title="Edit Disposisi">
